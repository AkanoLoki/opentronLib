...
metadata = {
    ...
    'description': 'Protocol to prepare, start, take 30-minute timepoints and quench into a Corning 3993 96 well fluorescence microplate. Adapted from JYChow@NUS',
    'apiLevel': '2.12'}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Interval between timepoints in minutes, must be greater or equal to 1 minute.
    intervalMinutes:float = 14.0
    ...
    for row in range(8):
        ...
        if 'E+' in rxnWells[row][2]:
            if lastE in 'E-':
                ...
                p300s.drop_tip()
                p300s.pick_up_tip()
            p300s.transfer(37.5, lysate, rxnWells[row][1], new_tip='never')
            lastE = 'E+'
        else:
            if lastE in 'E+':
                # Change tip upon content switch
                p300s.drop_tip()
                p300s.pick_up_tip()
            p300s.transfer(37.5, lysBuf, rxnWells[row][1], new_tip='never')
            lastE = 'E-'

    p300s.drop_tip()

    # Rxn buffer acidification of lysate
    # Pipette 240uL of reaction buffer in each well A1-H1, use 8 new tips.
    p300s.transfer(300, rBuf, rxnWells[0][0].columns_by_name()['1'])

    # Pause before starting rxn
    protocol.pause(
        'Check if mixture and plate are ready. Resuming will start pipetting substrate.')

    # Add substrates, takes approx 12s
    p300m.pick_up_tip()
    ...
    p300m.blow_out()
    # Timepoint 1-10
    delayTimes = [16.85, 35.3, 35.3, 35.3,
                  35.3, 35.3, 35.3, 35.3,
                  35.3, 35.3, 35.3, 35.3]
    for tp in range(12):
        # delay first
        protocol.delay(delayTimes[tp], intervalMinutes-1)
        # transfer to C3694
